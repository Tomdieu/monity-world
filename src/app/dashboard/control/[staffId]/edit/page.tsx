...
export function EditUserPage() {
  ...
  const RoleAndAccessForm = () => {
    ...
    const handleAccessChange = (
      key: string,
      type: "limited" | "fullAccess",
      checked: boolean
    ) => {
      if (checked) {
        const fieldPath = `accessLevels.${key}.${
          type === "limited" ? "fullAccess" : "limited"
        }` as const;
      ...
      <Form {...form}>
        ...
        >
          <div className="w-full flex items-center justify-end">
            <h1 className="font-bold text-lg">Edit staff info</h1>
          </div>
          {step === 1 ? (
