...
function DeclineKycModal({ onOpenChange, open, onSubmit }: DeclineKycModalProps) {
    ...
    return (
        <Credenza open={open} onOpenChange={onOpenChange}>
            <CredenzaContent className='pt-11'>
                <CredenzaTitle className='text-primary text-left px-11'>Kindly provide explanation to user as to why their KYC was declined.</CredenzaTitle>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSubmit)} className='flex flex-col gap-10 px-11'>
                        ...
                        <div className='flex items-center justify-center gap-8'>
                            <Button onClick={() => onOpenChange(false)} size={"lg"} variant={"outline"} className='border-primary text-primary' type='button'>I wish to check again</Button>
                            <Button size={"lg"} variant={"destructive"}>Confirm KYC decline</Button>
                        </div>
                    </form>
                </Form>
            </CredenzaContent>
